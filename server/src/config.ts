...
const envSchema = z.object({
  ...
  APP_ID: z.string().min(1).default('wxa64faf2abab7e388'),
  WECHAT_CLOUD_ENV_ID: z.string().min(1).default('prod-d5g4tznceeecbaf39'),
  WECHAT_CLOUD_SERVICE: z.string().min(1).default('express-bonj'),
  ...
  OBJECT_BUCKET: z.string().min(1).default('7072-prod-d5g4tznceeecbaf39-1346314817'),
  COS_REGION: z.string().min(1).default('ap-shanghai'),
  SUBSCRIBE_TEMPLATE_ID: z.string().min(1).default('fTNBBNLftez9i-ZcduqQ98r3az3IE2IZOWGLqnyUT5s'),
  ...
  AUTO_MIGRATE: z.enum(['true', 'false']).default('true').transform((value) => value === 'true'),
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env): AppConfig {
  ...
  return {
    ...
    port: env.PORT,
    appId: env.APP_ID,
    cloudEnvId: env.WECHAT_CLOUD_ENV_ID,
    cloudService: env.WECHAT_CLOUD_SERVICE,
    mysql: {
      host: address.host,
      port: address.port,
      user: mysqlUser!,
    ...
    objectBucket: env.OBJECT_BUCKET,

...
最后验证日期：2026-07-25。“代码完成”不等于“真实云/真机已验收”。

| ID | 状态 | 本次交付/证据 | 未完成的外部动作 |
|---|---|---|---|
| P0-01 | 代码完成 | Git `main`、baseline commit/tag、忽略策略、SHA-256 基线、秘密扫描、CODEOWNERS 模板 | 配置远程仓库、真实 owner 和分支保护 |
| P0-02 | 代码完成 | Node/npm 锁定，三项目 `npm ci`，独立测试边界，`verify:all` | CI 远程 run 证据 |
...
| P1-01 | 代码完成 | 固定 SHA 的 CI、MySQL 5.7 service、周审计、手工 release workflow、Dependabot | 仓库 secrets/environment approval、staging CD 与生产人工审批 |
| P1-02 | 代码基线完成 | 前端 42、后端 47、函数 6 项测试，覆盖率门禁、CI 迁移重放/不变量 | 本机 Docker daemon 不可用；待 CI 真实 MySQL/容器 run |
...
| P2-02 | 代码完成 | 周审计、SBOM、Node/npm/action 锁定、hash 制品 | Docker base digest 因本机无法访问 registry 尚未锁定；需在 CI 查询并更新 |
...
本机 Docker daemon 未运行，因此不把 TypeScript/单元测试成功写成 MySQL 5.7、Docker、CloudBase、COS 或真机已通过。
